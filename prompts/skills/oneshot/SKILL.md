...
Outer loop: `sdp-orchestrate` drives phases. You execute @build and @review inline.

## Rules

1. **Get next action** — Run `sdp-orchestrate --feature F{XX} --next-action`. Parse the JSON output.
2. **Execute phase** — For `build`: run @build {ws_id}, commit, then `sdp-orchestrate --feature F{XX} --advance --result $(git rev-parse HEAD)`. For `review`: run @review F{XX}, fix P0/P1 until approved, then `sdp-orchestrate --feature F{XX} --advance`.
