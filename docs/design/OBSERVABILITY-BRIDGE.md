...
| `sdp.model_id` | string | Model identifier if AI-generated | `claude-sonnet-4-20250514` |
...
def login(username: str, password: str) -> Token:
    ...
    span.set_attribute("sdp.model_id", "claude-sonnet-4-20250514")
...
```json
{
  ...
  "attributes": {
    ...
    "sdp.model_id": "claude-sonnet-4-20250514",
...
def validate_password(password: str) -> bool:
    ...
    # No model_id, evidence_id, or workstream_id
...
func (p *SDPProvenanceProcessor) OnStart(span, parent SpanContext) {
    ...
    if evidence != nil {
        ...
        span.SetAttribute("sdp.model_id", evidence.ModelID)
...
```sql
SELECT sdp.model_id, COUNT(*) as error_count
FROM production-traces
WHERE error = true
  AND sdp.ai_generated = true
GROUP BY sdp.model_id
...
```yaml
type: table
query: SELECT sdp.model_id, COUNT_IF(error=true) as errors FROM traces WHERE sdp.ai_generated=true GROUP BY sdp.model_id ORDER BY errors DESC LIMIT 10
```
...
sdp.model_id:claude-sonnet-4 -> sdp.model_id:claude-sonnet-4
...
```json
{
  ...
  "requests": [
    {
      "q": "sum:trace.errors{env:prod,sdp.ai_generated:true} by {sdp.model_id}.as_count()"
    }
  ]
}
```
...
```json
{
  ...
  "targets": [
    {
      "expr": "histogram_quantile(0.95, sum(rate(traces_span_latency_bucket{sdp_ai_generated=\"true\"}[5m])) by (le, sdp_model_id))",
      "legendFormat": "{{sdp_model_id}}"
    }
  ]
}
```
...
- `sdp.model_id` (model name, no user data)
...
**Acceptance Criteria:**
- Spans include `sdp.ai_generated`, `sdp.model_id`, `sdp.evidence_id`
