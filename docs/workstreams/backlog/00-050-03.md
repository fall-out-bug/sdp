...
Provide Go wrapper around Beads CLI for task tracking. Read `.beads-sdp-mapping.jsonl` for WS ID ↔ Beads ID mapping. Auto-sync on git commit.

**What must WORK after completing this WS:**
- `bd ready` → list available tasks
- `bd show <id>` → get task details
- `bd update <id> --status <status>` → update task status
- Mapping WS ID ↔ Beads ID
...
- [ ] AC4: Mapping WS ID ↔ Beads ID works
...
**Builds on F041:**
- ✅ `sdp-plugin/internal/hooks/` exists (from F041-05)
- 🆕 Add Beads sync to existing hooks
