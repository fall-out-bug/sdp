...
- [x] AC1: Remove self-referential submodule from `.gitmodules`
- [x] AC2: Clean up `.sdp/.sdp` directory if present
- [x] AC3: Document alternative dogfooding approach (direct path reference vs submodule)
- [x] AC4: Remove `poetry.lock` or move to explicit `legacy/` directory
- [x] AC5: Audit `quality-gate.toml` — remove Python-specific references or mark as language examples (completed in WS-05)
- [x] AC6: Fix `go install` path in PROTOCOL.md and any other docs
- [x] AC7: Verify `git clone --recursive` works cleanly after cleanup
