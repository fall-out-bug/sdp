...
ws_id: "00-056-08"
feature_id: "F056"
title: "Refactor parse.go to ≤200 LOC"
status: "completed"
priority: "P3"
depends_on: []
blocks: []
...
# 00-056-08: Refactor parse.go to ≤200 LOC

## Goal

### 🎯 Goal

Reduce `sdp-plugin/cmd/sdp/parse.go` to ≤200 lines to satisfy the project quality gate. Move helper functions into a new file in the same package (`main`) so parse.go is ≤200 LOC and CLI behavior is unchanged.
...
- [x] AC1: parse.go ≤200 LOC (counted by `wc -l`) — 71 LOC
...
|------|--------|
| Pre-build | ✅ PASSED |
| Guard | ✅ Activated 00-056-08 |
| Refactor | parse.go → 71 LOC; parse_helpers.go added (139 LOC) |
| Build/Tests | ✅ go build; TestParseCommand, TestParseRun PASS |
| Post-build | ✅ go test ./... -short |
| AC1 | parse.go 71 LOC (≤200) |
