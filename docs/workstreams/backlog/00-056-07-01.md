...
ws_id: "00-056-07"
feature_id: "F056"
title: "Refactor skill.go to ≤200 LOC"
status: "backlog"
priority: "P3"
depends_on: []
blocks: []
...
# 00-056-07: Refactor skill.go to ≤200 LOC

## Goal

### 🎯 Goal

Reduce `sdp-plugin/cmd/sdp/skill.go` to ≤200 lines to satisfy the project quality gate. Move subcommand implementations into separate `skill_*.go` files in the same package (`main`) so each file is ≤200 LOC and CLI behavior is unchanged.
...
- [x] AC1: skill.go ≤200 LOC (counted by `wc -l`) — 38 LOC
...
|------|--------|
| Pre-build | ✅ PASSED |
| Guard | ✅ Activated 00-056-07 |
| Refactor | skill.go → 38 LOC; skill_record.go, skill_validate.go, skill_checkall.go, skill_list.go, skill_show.go added |
| Build/Tests | ✅ go build ./...; all skill tests pass |
| Post-build | ✅ go test ./... -short |
| AC1 | skill.go 38 LOC (≤200) |
| AC2 | list, show, validate, check-all, record verified |
| AC3 | Same flags and behavior |
| AC4 | TestSkillCommand, TestSkillValidateCmd, TestSkillCheckAllCmd, TestSkillListCmd, TestSkillShowCmd — PASS |
