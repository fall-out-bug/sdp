...
Complete phantom CLI removal per Phase 0 exit criteria. Replace `sdp guard context check`, `branch check`, `complete`, `finding list/resolve` with checkpoint-based validation and `bd list`. Aligned with oneshot-autonomous-design: defensive branch check via checkpoint (not via sdp guard context go).
