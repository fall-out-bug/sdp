...
```yaml
# Packages with errcheck disabled (current):
...
This means unchecked errors — the #1 source of silent failures in Go — are not detected in the majority of production code. The linter exists but provides false confidence.
...
- [ ] AC1: Catalog all current exclusion rules with justification status (justified/unjustified)
- [ ] AC2: Remove at least 50% of `errcheck` exclusions (fix underlying unchecked errors)
- [ ] AC3: Remove at least 50% of `gocognit` exclusions (leverage file splits from WS-09)
- [ ] AC4: Fix unchecked errors exposed by exclusion removal (add proper error handling)
- [ ] AC5: All tests pass after exclusion removal
- [ ] AC6: Document remaining justified exclusions with inline `//nolint` + reason
- [ ] AC7: Add lint health metric to baseline report
