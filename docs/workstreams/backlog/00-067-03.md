...
- [x] AC1: Audit all adapter configs for broken/outdated references
- [x] AC2: Fix `.opencode/opencode.json` agent prompt paths to use canonical `prompts/skills/`
- [x] AC3: Add missing agent definitions to OpenCode config (added debugger, visionary, analyst, fixer)
- [x] AC4: Fix `.cursor/worktrees.json` to reference Go setup instead of Poetry
- [x] AC5: Add CI validation that adapter configs reference existing files
- [x] AC6: Document adapter configuration patterns in `docs/reference/configuration.md` → see DEVELOPMENT.md
- [x] AC7: All adapter configs pass path validation without warnings
