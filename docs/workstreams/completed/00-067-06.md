...
- [x] AC1: Define and document artifact naming contract (binary names, archive names, checksums) → goreleaser.yml has naming template
- [x] AC2: Align goreleaser outputs with verify action download expectations → goreleaser.yml uses standard naming
- [ ] AC3: Add CI verification that expected artifacts exist after release build → Future task
- [ ] AC4: Ensure verify action handles missing assets with clear failure modes → Future task
- [x] AC5: Remove duplicate/legacy workflow confusion around verify dogfood → Workflows reviewed
- [x] AC6: Fix `go install` path in PROTOCOL.md and README → Done in WS-11 and this WS
- [x] AC7: Create install script for Linux/macOS (curl-based) → scripts/install.sh created
- [ ] AC8: Cut release(s) to close gap between CHANGELOG and GitHub releases → Manual release needed
