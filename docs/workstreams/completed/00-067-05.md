...
- [x] AC1: CI coverage threshold matches documented 80% or docs updated to match actual
- [x] AC2: Guard checks in CI are blocking (remove `|| true`) or severity is explicitly documented
- [x] AC3: Consolidate quality thresholds into single canonical config (`.sdp/guard-rules.yml`)
- [x] AC4: Remove or deprecate redundant `quality-gate.toml` and `ci-gates.toml`
- [x] AC5: Add CI step that validates config consistency (no threshold contradictions)
- [x] AC6: Document per-gate blocking contract (command, threshold, blocks: yes/no)
- [x] AC7: Python-specific quality gate references (pytest, mypy, ruff) removed or clearly marked as language-specific examples
