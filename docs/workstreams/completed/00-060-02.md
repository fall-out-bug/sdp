...
- [x] AC1: `sdp contract generate --features F054,F055` creates contract from shared boundaries
- [x] AC2: Contract YAML includes: type name, fields, required_by (which features), status
- [x] AC3: Contract placed in `.contracts/<shared-type>.yaml`
- [x] AC4: `sdp contract lock` locks contract (prevents modification during implementation)
- [x] AC5: Extend existing contract system (already has synthesize, lock, validate)
- [x] AC6: Synthesis engine resolves field conflicts between features
