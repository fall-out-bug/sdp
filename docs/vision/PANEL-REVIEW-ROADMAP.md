...
2. **Usage governance infrastructure в Phase 1.** Нужны metering и usage visibility для predictability
3. **Usage transparency с первого дня.** Ясные лимиты и наблюдаемость вместо скрытого поведения
...
2. **Team/collaboration features → P1.** Shared templates, conflict detection. Single-player не закрывает командные сценарии
...
2. **Стабильный OSS-контракт важнее резких licensing pivots.** "Free then restrict" → community riot
3. **Public repo должен иметь цельный OSS story для core-слоёв.** Прозрачные границы и предсказуемый adoption
...
3. **Добавить `sdp incident`.** Prod сломался → "это AI-сгенерировано?" + "какая верификация прошла?" Forensic trace от git commit назад через всю SDP цепочку. Критичная high-assurance фича
...
- Team features P3 → должны быть **P1** (командные workflow критичны)
- Usage governance infrastructure отсутствует → нужен в **Phase 1**
...
### Adoption targets
- ≥50 активных OSS run/week к month 4
- ≥200 активных OSS run/week к month 8
- ≥500 активных OSS run/week к month 12
...
| Usage governance + metering visibility | Collison | P1 |
| Team collaboration | Masad | P1 |
| Model selection policy | Karpathy | P1 |
| Failure mode UX | PG | P1 |

## Hashimoto's OSS adoption формула

> Open protocol + predictable public tooling contracts.
> Community строит интеграции вокруг стабильного OSS интерфейса и workflow-гарантий.
